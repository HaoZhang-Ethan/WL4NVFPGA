...
void SetupVPR(INP t_options *Options, INP boolean TimingEnabled,
	...
	/* init default filenames 以下几个操作都是初始化一些参数的名字*/
...
//设置计算的参数，可以仔细看看
	...
	*Segments = Arch->Segments;
	...
	SetupSwitches(*Arch, RoutingArch, Arch->Switches, Arch->num_switches);//加载switches数据
	SetupRoutingArch(*Arch, RoutingArch);//检查已经完成，仅仅是复制值
