...
void vpr_init(INP int argc, INP char **argv, OUTP t_options *options,
	...
	/* Verify settings don't conflict（冲突的） or otherwise not make sense */
...
void vpr_init_pre_place_and_route(INP t_vpr_setup vpr_setup, INP t_arch Arch) {
	...
	if (vpr_setup.Operation == TIMING_ANALYSIS_ONLY) {
		...
		num_used = (int *) my_malloc(num_types * sizeof(int));
		num_phy = (int *) my_malloc(num_types * sizeof(int));
		for(i = 0; i < num_types; i++){
			num_used[i] = num_blocks_type[i];
			num_phy[i] = num_instances_type[i];
		}
...
void vpr_pack(INP t_vpr_setup vpr_setup, INP t_arch arch) {
	...
	/* If needed, estimate 估计 inter-cluster delay. Assume the average routing hop goes out of 
...
//路由时，从输出引脚出来，接到wire上，然后通过wire接到另一个block的输入上
	if (vpr_setup.PackerOpts.timing_driven
		...
		opin_switch_del = get_switch_info(arch.Segments[0].opin_switch,
				Tdel_opin_switch, R_opin_switch, Cout_opin_switch);//返回开关的总的延迟
		...
		Rmetal = arch.Segments[0].Rmetal;//R metal 不知道是干什么的
		Cmetal = arch.Segments[0].Cmetal;

		/* The delay of a wire with its driving switch is the switch delay plus the 
		 product of the equivalent resistance and capacitance experienced by the wire.线的延迟是驱动开关延迟和线的电阻电容造成的延迟的和 */
//进一步计算延迟
#define WIRE_SEGMENT_LENGTH 4
		...
		inter_cluster_delay = 4
				* (first_wire_seg_delay + second_wire_seg_delay
						+ wtoi_switch_del); /* multiply by 4 to get a more conservative estimate 保守估计 */
	}
//以上是计算一些延迟信息。。。。。。。
...
//
	if (block != NULL) {
		for (i = 0; i < num_blocks; i++) {
			...
			free(block[i].nets);
			free(block[i].name);
			printf("fjhdsaljfkldsajfkl;dsajn\n");
		}
	}
...
void vpr_free_all(INOUTP t_arch Arch, INOUTP t_options options,
	...
	if (vpr_setup.RouterOpts.doRouting) {
		printf("before free route\n");
		free_route_structs();
		printf("after free route\n");
	}
	printf("before free trace\n");
	free_trace_structs();
	printf("after free trace\n");
	vpr_free_vpr_data_structures(Arch, options, vpr_setup);
	printf("after free date\n");
	if (has_printhandler_pre_vpr == FALSE) {
		printf("before print\n");
		PrintHandlerDelete();
		printf("after print\n");
