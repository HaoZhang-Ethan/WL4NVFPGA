...
//zh_lab
#include<string.h>
static void get_forbid_mem(char * mem_path);
...
static void initial_placement(enum e_pad_loc_type pad_loc_type,
	...
	strcpy (log_pin_path,"/home/zhlab/BRAM/s_run_e_3/");
...
static void check_place(float bb_cost, float timing_cost, 
	...
	for (tmp_i = 0; tmp_i <= nx + 1; tmp_i++) {
		for (tmp_j = 0; tmp_j <= ny + 1; tmp_j++) {
			if(grid[tmp_i][tmp_j].forbid == 1)
			{
				grid[tmp_i][tmp_j].usage = 0;
				for (tmp_k = 0; tmp_k < grid[tmp_i][tmp_j].type->capacity; tmp_k++) {
					grid[tmp_i][tmp_j].blocks[tmp_k] = EMPTY;
				}
			}
...
static void get_forbid_mem(char * mem_path)
{
	...
	while (fgets(buf, MAX_LINE, (FILE*)fp) != NULL)
	{
			...
			{
				if(istr.num > 2){
					int tmp_x = char_to_int(istr.str[0]);
					int tmp_y = char_to_int(istr.str[1]);
					grid[tmp_x][tmp_y].forbid = 1;
					// vpr_printf(TIO_MESSAGE_INFO,"%d-%d",tmp_x,tmp_y) ;
				}
