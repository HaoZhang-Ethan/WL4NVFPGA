...
static float load_phy_info_to_log_2(BRAMS_phy *p_phy_brams, BRAMS_log *p_log_brams, int phy_bram_id, int log_bram_id, long int up_limit, addr_pin *tmp_D_port, int *error,float *p_bed_cost);
unsigned long long case_num_max(int num);
void perm(int deep, int n, int valid_num, Levels *l_vecs, int valid_add_num, int add_range, BRAMS_log *p_log_brams, int log_bram_id, int *error, int up_limit);
void comb(int deep, int m, int n, int *p_perm, Levels *l_vecs, int valid_add_num, int add_range, BRAMS_log *p_log_brams, int log_bram_id, int *error, int up_limit);
int complement_set(int *p_A, int num_a, int *p_B, int num_b, int *p_C);
float computer_cost(Levels *l_vecs, int valid_add_num, int add_range, BRAMS_log *p_log_brams, int log_bram_id, int *error, int up_limit);
	...
	if (abort_swap == FALSE)
	{
		...
		else
		{
			...
			if (block_id_to != OPEN)
			{
				...
				int error = 0;
				float bed_cost = 0;
				float cost_from_new = load_phy_info_to_log_2(placer_opts.p_phy_brams, placer_opts.p_log_brams, phy_bram_id_to, log_bram_id_from, placer_opts.up_limit, &from_to, &error,&bed_cost);
				if (error == 1)
				{
				...
				error = 0;
				float cost_to_new = load_phy_info_to_log_2(placer_opts.p_phy_brams, placer_opts.p_log_brams, phy_bram_id_from, log_bram_id_to, placer_opts.up_limit, &to_from, &error,&bed_cost);
				if (error == 1)
				{
					keep_switch = REJECTED;
				}
...
int complement_set(int *p_A, int num_a, int *p_B, int num_b, int *p_C)
{
	int a[num_a];
	int b[num_b];
	memcpy(a, p_A, sizeof(int) * num_a);
	memcpy(b, p_B, sizeof(int) * num_b);
	int i, j;
	int bb[15];
	int y = 0;
	int m, n;
 ...
 * @param n 要查找的组合的长度
 */
void comb(int deep, int m, int n, int *p_perm, Levels *l_vecs, int valid_add_num, int add_range, BRAMS_log *p_log_brams, int log_bram_id, int *error, int up_limit, float *p_cost, addr_pin *tmp_D_port,float *p_bed_cost)
{
	int i = 0;
	if (deep > m) // 越界递归结束
		return;
	if (results_end == n) // 找到结果，打印，递归结束
	{
		int add_index[m];
		memset(add_index, 0, sizeof(int) * m);
		// printf("\ncomb_is\n");
		...
		{
			int tmp_i = results[i];
			int tmp_val = p_perm[i];
			// printf("-- %d --%d\n\n\n", tmp_i, tmp_val);
			*(add_index + tmp_i) = tmp_val;
		}
		if (m - n > 0)
		{
			int tmp_C[m - n];
			int compl_case_num = zh_pow(2, m - n);
			int tmp_comp_num = 0;
			for (tmp_comp_num = 0; tmp_comp_num < compl_case_num; tmp_comp_num++)
			{
				int tmp_A[m - n];
				d_2_b(tmp_comp_num, m - n, tmp_A);
				memset(tmp_C, 0, sizeof(int) * m - n);
				complement_set(results, n, array, m, tmp_C);
				// printf("compl_is\n");
				// for (i = 0; i < m - n; i++)
				// {
				//     printf("%d,", tmp_C[i]);
				...
				for (i = 0; i < m; i++)
				{
					...
					float zero_ratio = 0;
					addr_pin tmp_pin = (*p_log_brams).BRAM_LIST[log_bram_id].C_port;
					int index = add_index[i];
					if (index == -1)
					{
						strcpy(tmp_name, "vcc");
						zero_ratio = 0;
					...
					{
						strcpy(tmp_name, tmp_pin.pins[index].pin_name);
						zero_ratio = (1 - tmp_pin.pins[index].pin_ratio);
					}
					else
					{
						vpr_printf(TIO_MESSAGE_INFO, "index error \n");
						exit(0);
					}
					strcpy((*l_vecs).l_vec[i].level_pin_name, tmp_name);
					(*l_vecs).l_vec[i].w_node = zero_ratio;
					(*l_vecs).l_vec[i].b_node = (1 - zero_ratio);
				}
				float cost = computer_cost(l_vecs, valid_add_num, add_range, p_log_brams, log_bram_id, error, up_limit);
				if (cost < (*p_cost))
				{
					(*p_cost) = cost;
					int tmp_pin_it = 0;
					for (tmp_pin_it = 0; tmp_pin_it < valid_add_num; tmp_pin_it++)
					{
						strcpy((*tmp_D_port).pins[tmp_pin_it].pin_name, (*l_vecs).l_vec[tmp_pin_it].level_pin_name);
					}
				}
				if ((*p_bed_cost) < cost )
				{
				...
				// vpr_printf(TIO_MESSAGE_INFO, "cost %f \n", cost);
			}
		}
		else
		{
	...
	comb(deep + 1, m, n, p_perm, l_vecs, valid_add_num, add_range, p_log_brams, log_bram_id, error, up_limit, p_cost, tmp_D_port,p_bed_cost); //向下一级递归
	results_end--;
	comb(deep + 1, m, n, p_perm, l_vecs, valid_add_num, add_range, p_log_brams, log_bram_id, error, up_limit, p_cost, tmp_D_port,p_bed_cost); //向下一级递归
}
 ...
 * @param n 数组最大长度
 */
void perm(int deep, int n, int valid_num, Levels *l_vecs, int valid_add_num, int add_range, BRAMS_log *p_log_brams, int log_bram_id, int *error, int up_limit, float *p_cost, addr_pin *tmp_D_port,float *p_bed_cost)
{
	int i;
	if (deep > n) // 越界递归结束
	{
	...
	if (deep == n) // 找到结果，打印，递归结束
	{
		...
		// }
		// printf("-----\n");
		comb(0, valid_num, n, p_perm, l_vecs, valid_add_num, add_range, p_log_brams, log_bram_id, error, up_limit, p_cost, tmp_D_port,p_bed_cost);
	...
	for (i = 0; i < n; i++)
	{
		if (is_used[i] == 0)
		{
			is_used[i] = 1;
			p_perm[deep] = array[i];
			perm(deep + 1, n, valid_num, l_vecs, valid_add_num, add_range, p_log_brams, log_bram_id, error, up_limit, p_cost, tmp_D_port,p_bed_cost);
			is_used[i] = 0;
		}
	}
}
...
static float load_phy_info_to_log_2(BRAMS_phy *p_phy_brams, BRAMS_log *p_log_brams, int phy_bram_id, int log_bram_id, long int up_limit, addr_pin *tmp_D_port, int *error ,float *p_bed_cost)
{
	...
	float cost = 900000000;
	perm(0, valid_real_pin_num, valid_add_num, &l_vecs, valid_add_num, add_range, p_log_brams, log_bram_id, error, up_limit, &cost, tmp_D_port,p_bed_cost);
	return cost;
	// return computer_cost(&l_vecs,valid_add_num,add_range, p_log_brams,log_bram_id,error,up_limit);
}