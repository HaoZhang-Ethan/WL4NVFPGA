 ...
 *
 * Limitations: Currently assumes that forced pack nets must be single-fanout（单扇出） as this covers all the reasonable architectures we wanted.
 More complicated（复杂） structures should probably be handled either downstream (general packing) or upstream (in tech mapping)
 *              If this limitation is too constraining（制约）, code is designed so that this limitation can be removed
 */
...
/**
 * Pre-pack atoms in netlist to molecules（分子）
...
t_pack_molecule *alloc_and_load_pack_molecules(
	...
	/* Find forced pack patterns 根据模式的优先级进行map*/
	/* Simplifying assumptions: Each atom can map to at most one molecule, use first-fit mapping based on priority of pattern */
	/* TODO: Need to investigate better mapping strategies than first-fit 调查比第一fit更好的映射策略 */
	for (i = 0; i < num_packing_patterns; i++) {
		best_pattern = 0;
