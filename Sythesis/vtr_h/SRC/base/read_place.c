...
void print_place(char *place_file, char *net_file, char *arch_file) {
	...
	char * pla_out = (char*) my_malloc ((name_len + 8)*sizeof(char));
	name_len = 0;
	while (1){
		if (full_circuit_name[name_len] == '\0')
			break;
		pla_out[name_len] = full_circuit_name[name_len];
		name_len ++;
	}

	pla_out[name_len] = '_';
	pla_out[name_len+1] = 'p';
	pla_out[name_len+2] = 'l';
	pla_out[name_len+3] = 'a';
	pla_out[name_len+4] = '.';
	pla_out[name_len+5] = 't';
	pla_out[name_len+6] = 'x';
	pla_out[name_len+7] = 't';
	pla_out[name_len+8] = '\0';
	...
	for (i = 0; i < num_blocks; i++) {
		if( strcmp( (*(block[i].type)).name,"memory" )==0 ){
			fprintf(fp, "%s ", block[i].name);
			fprintf(fp, "%d %d\n", block[i].x, block[i].y);
		}
	}
