...
#define wl_with_pro 2 //flag，如果为1，则表示带考虑性能的WL，为0就是CLB不固定的WL，为2是CLB固定的。
...
static enum swap_result assess_swap(float delta_c, float t, float p1, float p2, int *if_goto, int if_init);

static boolean find_to(int b_from, int x_from, int y_from, t_type_ptr type, float rlim, int *x_to, int *y_to,char* block_name);
...
static int exist_config_file = 0;
	...
	while (exit_crit(t, cost, annealing_sched) == 0) {
		...
		for (inner_iter = 0; inner_iter < move_lim; inner_iter++) {
			swap_result = try_swap(t, &cost, &bb_cost, &timing_cost, rlim,
					...
					old_region_occ_y, 
					placer_opts.place_algorithm, placer_opts.timing_tradeoff,
					inverse_prev_bb_cost, inverse_prev_timing_cost, &delay_cost,0);
...
Lable:
	...
	if (!find_to(b_from, x_from, y_from, block[b_from].type, rlim, &x_to,
...
// 计算这次交换是否带来寿命问题：
	...
	float p1 = 1, p2 = 1;//p1,p2 分别代表的是交换带来的问题。。。
	if (wl_with_pro == 1 && exist_config_file == 1 && strcmp(block[b_from].type->name,"memory") == 0){
		int ilbr_from = 0;
		for( ilbr_from = 0;ilbr_from < used;ilbr_from++){
			if( strcmp(lbr[ilbr_from].name,block[b_from].name)==0 )
				break;
		}
	
		assert( strcmp(lbr[ilbr_from].name,block[b_from].name)==0 );

		int ipbr_from = 0;
		for( ipbr_from = 0;ipbr_from < phy;ipbr_from++){
			if(x_from == pbr[ipbr_from].x && y_from == pbr[ipbr_from].y)
				break;
		}
		assert(x_from == pbr[ipbr_from].x && y_from == pbr[ipbr_from].y);

		int ipbr_to = 0;
		for (ipbr_to = 0; ipbr_to < phy; ipbr_to ++){
			if(x_to == pbr[ipbr_to].x && y_to == pbr[ipbr_to].y)
				break;
		}
		...
		if (grid[x_to][y_to].blocks[0] >= 0){
			int ilbr_to = 0;
			int b_to = grid[x_to][y_to].blocks[0];

			for( ilbr_to = 0;ilbr_to < used;ilbr_to++){
				if( strcmp(lbr[ilbr_to].name,block[b_to].name)==0 )
					break;
			}
	...
	if (abort_swap == FALSE) {
		...
		keep_switch = assess_swap(delta_c, t, p1, p2,&if_goto,if_init);
		...
		if (if_goto == 1 && if_init == 0 && wl_with_pro == 1){
			aall++;
			if (aall < 10000)
				goto Lable;
		}
...
static boolean find_to(int b_from, int x_from, int y_from, t_type_ptr type, float rlim, int *x_to, int *y_to,char* block_name) {
	...
	/*	else if( ( ( (*x_to)==10 && (*y_to)==7 ) ||( (*x_to)==2 && (*y_to)==7 )||( (*x_to)==10 && (*y_to)==13 ) ||( (*x_to)==18 && (*y_to)==7 ) ||( (*x_to)==2 && (*y_to)==13 ) ||( (*x_to)==10 && (*y_to)==1 )||( (*x_to)==2 && (*y_to)==1 )||( (*x_to)==18 && (*y_to)==13 ) ) && strcmp(type->name,"memory") ==0) {
		...
		else if(wl_with_pro != 1 && location_bool(type->name,block_name,(*x_to),(*y_to),0)){
			is_legal = FALSE;
		}

		else if(wl_with_pro != 1 && (grid[*x_to][*y_to].blocks[0]>=0)&&location_bool(type->name,block[grid[*x_to][*y_to].blocks[0]].name,x_from,y_from,0)){
			is_legal = FALSE;
		}		
	...
	*if_goto = 0;
	if (delta_c <= 0) {
		...
		if(if_init == 1){
			...
			if((p1 > 0.4 || 1 - a1 + (-delta_c / t ) > 0.4) && (p2 > 0.4 || 1 - p2 + (-delta_c / t ) > 0.4) && p1 > 0.2 && p2 > 0.2 )
				accept = ACCEPTED;
			else{
				accept = REJECTED;
				*if_goto = 1;
	...
	if(if_init == 1){
		...
		if (prob_fac > fnum && p1 > 0.5 && p2 > 0.5) {
			accept = ACCEPTED;
		} else {
