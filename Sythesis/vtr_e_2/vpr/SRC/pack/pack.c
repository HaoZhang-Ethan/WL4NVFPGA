...
void try_pack(INP struct s_packer_opts *packer_opts, INP const t_arch * arch,
	...
	list_of_packing_patterns = alloc_and_load_pack_patterns(
			&num_packing_patterns);// Find all packing patterns in architecture
...
float get_switch_info(short switch_index, float &Tdel_switch, float &R_switch, float &Cout_switch) {
	/* Fetches delay, resistance and output capacitance of the switch at switch_index. 取出延迟，电阻，输出电容
	Returns the total delay through the switch. Used to calculate inter-cluster net delay. 返回通过开关总的延迟*/
