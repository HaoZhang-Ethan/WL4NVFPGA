...
static enum swap_result try_swap(float t, float *cost, float *bb_cost, float *timing_cost,
		...
		float *delay_cost,struct s_placer_opts placer_opts);
...
static float starting_t(float *cost_ptr, float *bb_cost_ptr,
		...
		enum e_place_algorithm place_algorithm, float timing_tradeoff,
		float inverse_prev_bb_cost, float inverse_prev_timing_cost,
		float *delay_cost_ptr,struct s_placer_opts placer_opts);
...
void try_place(struct s_placer_opts placer_opts,
	...
	strcpy(phy_pin_path, placer_opts.e_path_info);
...
static enum swap_result try_swap(float t, float *cost, float *bb_cost, float *timing_cost,
	...
	if (abort_swap == FALSE) {
		...
		//zh_lab
		if (block[b_from].type->index == 4)// b_from 是BRAM
		{

			/* 1 -> move accepted, 0 -> rejected. */
			keep_switch = assess_swap(delta_c, t);//性能优先的布局 退火评价

			// 均衡策略
			int tmp_x_from = x_from;
			int tmp_y_from = y_from;
			int tmp_z_from = z_from;
			...
			int block_id_from = grid[tmp_x_from][tmp_y_from].blocks[tmp_z_from];//找到from_block的对应block的ID
			assert(block_id_from != -1);//要换的块 from 一定是有逻辑的
			
			int block_id_to = grid[tmp_x_to][tmp_y_to].blocks[tmp_z_to];		//找到from_to的对应block的ID

			// TODO:
			// 需要加自检
			int log_bram_id_from = block[block_id_from].log_bram_ID;		//从Block 对应到 log ID
			int phy_bram_id_from = grid[tmp_x_from][tmp_y_from].BRAM_ID;	//从grid 对应到 phy ID
			int phy_bram_id_to = grid[tmp_x_to][tmp_y_to].BRAM_ID;			//从grid 对应到 phy ID
			int log_bram_id_to = -1;
			if (block_id_to != OPEN)
			{
				log_bram_id_to = block[block_id_to].log_bram_ID;
			}
			addr_pin from_to_D = {0};
			addr_pin to_from_D = {0};


			if (block_id_to != OPEN)//假如to有逻辑
			{
				...
				old_cost = cost_from_old + cost_to_old;
				int error = 0;
				float cost_to_new = 0.0;
				float cost_from_new = 0.0;
				cost_from_new = load_phy_info_to_log(placer_opts.p_phy_brams, placer_opts.p_log_brams, phy_bram_id_to, log_bram_id_from, placer_opts.up_limit, &from_to_D, &error,placer_opts.min_ratio);
				if (error == 1)
				{
				...
				{
					cost_to_new = load_phy_info_to_log(placer_opts.p_phy_brams, placer_opts.p_log_brams, phy_bram_id_from, log_bram_id_to, placer_opts.up_limit, &to_from_D, &error,placer_opts.min_ratio);
					if (error == 1)
					{
						keep_switch = REJECTED;
					}
				...
				new_cost = cost_from_new + cost_to_new;
				...
				{
					// vpr_printf(TIO_MESSAGE_INFO, "%d-%d\t%f\n", tmp_x_from, tmp_y_from,cost_from_old,cost_to_old);
					// vpr_printf(TIO_MESSAGE_INFO, "%d-%d\t%f\n", tmp_x_to, tmp_y_to,cost_from_new,cost_to_new);
					memcpy(&(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id_from].D_port,&from_to_D,sizeof(addr_pin));
					memcpy(&(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id_to].D_port,&to_from_D,sizeof(addr_pin));
					(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id_from].cost = cost_from_new;
					(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id_to].cost = cost_to_new;
				}
			}
			else
			{
				...
				int error = 0;
				cost_from_new = load_phy_info_to_log(placer_opts.p_phy_brams, placer_opts.p_log_brams, phy_bram_id_to, log_bram_id_from, placer_opts.up_limit, &from_to_D, &error,placer_opts.min_ratio);
				old_cost = cost_from_old;
				new_cost = cost_from_new;
				if (error == 1)
				{
					keep_switch = REJECTED;
				}
				...
				if (keep_switch == ACCEPTED)
				{
					// vpr_printf(TIO_MESSAGE_INFO, "%d-%d\t%f\n", tmp_x_from, tmp_y_from,cost_from_old);
					// vpr_printf(TIO_MESSAGE_INFO, "%d-%d\t%f\n", tmp_x_to, tmp_y_to,cost_from_new);
					memcpy(&(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id_from].D_port,&from_to_D,sizeof(addr_pin));
					(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id_from].cost = cost_from_new;
				}
	...
	for (iblk = 0; iblk < num_blocks; iblk++) {
		...
		if (!(block[iblk].type == IO_TYPE && pad_loc_type == USER)) {
			...
			if (block[iblk].type->index == 4)
			{
				...
				assert(log_bram_id < placer_opts.p_log_brams->BRAM_num);
				...
				do
				{
					...
					(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id].cost = load_phy_info_to_log(placer_opts.p_phy_brams, placer_opts.p_log_brams, phy_bram_id, log_bram_id, placer_opts.up_limit, &tmp_D_port, &error,placer_opts.min_ratio);
					try_num--;
					// vpr_printf(TIO_MESSAGE_INFO,"    %d    %f\n",error,(*placer_opts.p_log_brams).BRAM_LIST[log_bram_id].cost);
					if (error == 1)
					{
						int a = 0;
					}
				...
				if (error == 1)
				{
					vpr_printf(TIO_MESSAGE_ERROR, "ERROR:NO BRAM for USED\n");
					exit(1);
				}
				else
				{
...
void update_mem_phy(BRAMS_phy *p_phy_brams, Array_s *p_Arrays, char *p_mem_path)
{
		...
		for (j = 0; j <= ny + 1; j++)
		{
			if (strcmp(grid[i][j].type->name, "memory") == 0)
			{
				if (k == 0 || m_flag == 6)
				{
					...
					memcpy((*(tmp_p_Arrays)).Array, tmp_Array, (sizeof(long int)*MAX_ROW_B*MAX_COL_B));
	...
	assert(BRAM_num == 208 );// zh_lab BRAM总数量
}
...
static float load_phy_info_to_log(BRAMS_phy *p_phy_brams, BRAMS_log *p_log_brams, int phy_bram_id, int log_bram_id, long int up_limit, addr_pin *tmp_D_port, int *error,float min_ratio)
{
	...
	// TODO:
	// need free
	tmp_init = (BRAM_log_info *)malloc(sizeof(BRAM_log_info));
	memcpy(tmp_init,&((*p_log_brams).BRAM_LIST[log_bram_id]),sizeof(BRAM_log_info));

	int add_range = (*tmp_init).add_range;
	Array_s tmp_Array;
	memcpy(tmp_Array.Array, (*(*p_phy_brams).BRAM_LIST[phy_bram_id].p_Array).Array, sizeof(Array_s));
	//读取cell信息，获取最小写单元的当前写次数
	for (add_counter = 0; add_counter < add_range; add_counter++)
	{
		int tmp_row = (*((*tmp_init).p_write_unit_vector + add_counter)).relocation_table_x;
		int col_num = (*tmp_init).con_width;
		int i = 0;
		long int cur_max_write_num = -1;
		for (i = 0; i < col_num; i++)
		{
			int tmp_col = (*((*tmp_init).p_write_unit_vector + add_counter)).relocation_table_y[i];
			assert((tmp_row < ROW_DEPTH*COL_WIDTH) && (tmp_col < CONFIG_WIDTH));
		...
		(*((*tmp_init).p_write_unit_vector + add_counter)).write_num = cur_max_write_num;

	}

	Levels l_vecs;
	int l_num = 0;
	int valid_add_num = (*tmp_init).valid_add_num;
	addr_pin *tmp_pin;
	tmp_pin = &(*tmp_init).C_port;

	int valid_real_pin_num = 0; //有效的实际连线数量
	for (l_num = 0; l_num < valid_add_num; l_num++)
	{
		if (strlen((*tmp_pin).pins[l_num].pin_name) > 6)//名字大于6 认为是有连接的引脚
		{
			valid_real_pin_num++;
		}
	}
	...
	for (l_num = 0; l_num < valid_add_num; l_num++)
	{
		...
		if (l_num == 0)
		{
			...
			for (tmp_i = 0; tmp_i < tmp_block_num; tmp_i++)
			{
				...
				if (tmp_i % 2 == 0)
				{
					for (tmp_j = tmp_i * tmp_unit_num; tmp_j < (tmp_i + 1) * tmp_unit_num; tmp_j++)
					{
						...
						w_wirte = w_wirte + (float)(*((*tmp_init).p_write_unit_vector + tmp_j)).write_num;
					}
				}
				else
				{
					for (tmp_j = tmp_i * tmp_unit_num; tmp_j < (tmp_i + 1) * tmp_unit_num; tmp_j++)
					{
						b_wirte = b_wirte + (float)(*((*tmp_init).p_write_unit_vector + tmp_j)).write_num;
					}
				}
			}
			float total_num = (w_wirte + 0.00001) + (b_wirte + 0.00001);
			...
			int v_add_num = 0;
			int tmp_v_add_num = 0;
			v_add_num = (*tmp_init).valid_add_num;
			float Diff = 1000.0;
			int Min_num_pos = -1;
			float tmp_diff = 0.0;
			...
			{
				vpr_printf(TIO_MESSAGE_ERROR,"level_0 no find pin_flag");
				exit(1);
			}
		...
		else
		{
			...
			for (tmp_i = 0; tmp_i < tmp_block_num; tmp_i++)
			{
				...
				if (tmp_i % 2 == 0)
				{
					for (tmp_j = tmp_i * tmp_unit_num; tmp_j < (tmp_i + 1) * tmp_unit_num; tmp_j++)
					{
						...
						w_wirte = w_wirte + coefficient * ((*((*tmp_init).p_write_unit_vector + tmp_j)).write_num);
					}
				}
				else
				{
					for (tmp_j = tmp_i * tmp_unit_num; tmp_j < (tmp_i + 1) * tmp_unit_num; tmp_j++)
					{
						...
						b_wirte = b_wirte + coefficient * ((*((*tmp_init).p_write_unit_vector + tmp_j)).write_num);
			...
			{
				l_vecs.l_vec[0].w_node = 1.0;
			}
			else
			{
				vpr_printf(TIO_MESSAGE_ERROR,"level_n no find pin_flag");
				exit(1);
			}
	...
	// //计算cost
	float cost = 0.0;
	cost = computer_cost(&l_vecs,valid_add_num,add_range,tmp_init,error,up_limit,min_ratio);
	free(tmp_init);
	return cost;
}
// TODO:
...
static float computer_cost(Levels *l_vecs, int valid_add_num, int add_range, BRAM_log_info *tmp_init, int *error, int up_limit ,float min_ratio)
{
	...
	for (tmp_i = 0; tmp_i < tmp_block_num; tmp_i++)
	{
		if (tmp_i % 2 == 0)
		{
			for (tmp_j = tmp_i * tmp_unit_num; tmp_j < (tmp_i + 1) * tmp_unit_num; tmp_j++)
			{
				...
				if (coefficient != 0)
				{
					if (((*((*tmp_init).p_write_unit_vector+tmp_j)).write_num > up_limit) && ((*tmp_init).log_ratio != 0))
					{
						(*error) = 1;
						return 0;
					}
	...
	*error = 0;
	return ((cost / v_write_num)*(*tmp_init).log_ratio);
}
