...
// TODO:
//最大地址线数量
#define MAX_ADDR_NUM 15
//pin 最大命名长度
#define MAX_name_len 200
#define MAX_write_unit 32768
#define MAX_BRAM_NUM 250
#define MAX_ROW_B 512
#define MAX_COL_B 64
typedef struct {
...
//write_unit
typedef struct BRAM_write_unit_
{
    int address;
    int unit_num;
    bool dirty;
...
{
    long int Array[MAX_ROW_B][MAX_COL_B];
}Array_s;

//单个物理BRAM
typedef struct BRAM_phy_info_{
	Array_s *p_Array;
}BRAM_phy_info;

//物理BRAM集合
typedef struct BRAMS_phy_{
	int BRAM_num;
	BRAM_phy_info BRAM_LIST[MAX_BRAM_NUM];
}BRAMS_phy;
...
//pin_词典
typedef struct pin_dict_ {
	int pin_num ;
	pin pins[1000];
}pin_dict;

//逻辑BRAM单个PORT
typedef struct addr_pin_ {
	int pin_num ;
	pin pins[MAX_ADDR_NUM];
}addr_pin;

//log 单个 BRAM
typedef struct BRAM_log_info_{
	char name[MAX_name_len];
	char mode[30];
	int valid_add_num;//使用地址线条数
	int add_range;//寻址范围
	int con_width;//可配置位宽度
	int is_dp;
	BRAM_write_unit * p_write_unit_vector;//最小写单元信息
	char name_port_a[MAX_name_len];
	addr_pin A_port;
	char name_port_b[MAX_name_len];
	addr_pin B_port;
	//双端RAM为了统一map流程虚拟为单端RAM
	char name_port_c[MAX_name_len];
	addr_pin C_port;		//以A为映射
	addr_pin D_port;		//预映射
	//最终顺序
	addr_pin A_port_;
	addr_pin B_port_;
...
//逻辑BRAM集合
typedef struct BRAMS_log_{
	int BRAM_num;
	BRAM_log_info BRAM_LIST[MAX_BRAM_NUM];
}BRAMS_log;
