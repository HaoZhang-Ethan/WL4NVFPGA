...
void SetupVPR(INP t_options *Options, INP boolean TimingEnabled,
	...
	char *str1=strstr(Options->CircuitName,PlacerOpts->benchmark_name);
	memcpy(PlacerOpts->e_path_info,Options->CircuitName,(str1-(Options->CircuitName)));

	//***************************


	len = strlen(Options->CircuitName) + 56; /* circuit_name.blif/0*/
	...
	if (Options->NetFile == NULL ) {
		len = strlen(Options->CircuitName) + 55; /* circuit_name.net/0*/
		if (Options->out_file_prefix != NULL ) {
			len += strlen(Options->out_file_prefix);
		}
	...
	if (Options->PlaceFile == NULL ) {
		len = strlen(Options->CircuitName) + 57; /* circuit_name.place/0*/
		if (Options->out_file_prefix != NULL ) {
			len += strlen(Options->out_file_prefix);
		}
	...
	if (Options->RouteFile == NULL ) {
		len = strlen(Options->CircuitName) + 57; /* circuit_name.route/0*/          //zh_lab   7  ->   57
		if (Options->out_file_prefix != NULL ) {
			len += strlen(Options->out_file_prefix);
		}
	...
	if (Options->ActFile == NULL ) {
		len = strlen(Options->CircuitName) + 57; /* circuit_name.route/0*/
		if (Options->out_file_prefix != NULL ) {
			len += strlen(Options->out_file_prefix);
		}
