...
//****************************
//zh_lab
void print_place_pin(char *place_file, BRAMS_log * p_log_brams) {
	FILE *fp;
	int i;
	strcat(place_file,"_pin");
	fp = fopen(place_file, "w");
	int num_blocks = (* p_log_brams).BRAM_num;
	fprintf(fp, "0 BRAMS_NUM %d\n", num_blocks);
	for (i = 0; i < num_blocks; i++) {
		fprintf(fp, "1 bram_name %s\n",(* p_log_brams).BRAM_LIST[i].name);
		fprintf(fp, "2 bram_mode %s",(* p_log_brams).BRAM_LIST[i].mode);
		fprintf(fp, "3 bram_is_dp %d\n",(* p_log_brams).BRAM_LIST[i].is_dp);
		fprintf(fp, "4 bram_valid_pin_num %d\n",(* p_log_brams).BRAM_LIST[i].valid_add_num);
		int valid_pin_num = (* p_log_brams).BRAM_LIST[i].valid_add_num;
		int j = 0;
		for(j = 0; j< valid_pin_num;j++)
		{
			fprintf(fp,"%s\n",(* p_log_brams).BRAM_LIST[i].D_port.pins[j].pin_name);
		}
	}
	fclose(fp);
}