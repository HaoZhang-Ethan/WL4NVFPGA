...
// #define PRINT_BRAM_INFO
	...
	char pin_dict_path[250];
	memset (pin_dict_path,NULL,sizeof(pin_dict_path));
	strcpy (pin_dict_path,placer_opts.e_path_info);
	strcat (pin_dict_path,placer_opts.benchmark_name);
	strcat (pin_dict_path,"/src/pin_dict");
	update_pin_dict(pin_dict_path,&pin_dict_inits);

	//创建BRAM phy cell 存储队列
	// TODO:
	// free
	Array_s * p_Arrays = (Array_s *)malloc(MAX_BRAM_NUM * sizeof(Array_s));
	...
	strcpy (log_pin_path,placer_opts.e_path_info);
	strcat (log_pin_path,placer_opts.benchmark_name);
	strcat (log_pin_path,"/src/pre/");
	strcat (log_pin_path,placer_opts.benchmark_name);
	strcat (log_pin_path,"_log_pin.info");
	//从_log_pin.info中载入BRAM信息
	load_log_BRAMS_info(log_pin_path,&log_brams);
...
// ****************************************
		...
		// 释放最小写单元空间
		int log_num = 0;
		for (log_num = 0;log_num < (log_brams).BRAM_num ;log_num++)
		{
			free((log_brams).BRAM_LIST[log_num].p_write_unit_vector);
		}
// ****************************************
...
void update_pin_dict(char *pin_dict_path, pin_dict *p_pin_dict_inits)
{
	...
	FILE *fp;
	int len;
	// vpr_printf(TIO_MESSAGE_INFO, "pin_dict path %s\n",pin_dict_path);
	if ((fp = fopen(pin_dict_path, "r")) == NULL)
	{
		vpr_printf(TIO_MESSAGE_INFO, "pin_dict path %s\n",pin_dict_path);
		vpr_printf(TIO_MESSAGE_ERROR, "fail to read pin_dict\n");
		exit(1);
	}
...
static void load_log_BRAMS_info(char * info_path ,BRAMS_log * p_log_brams)
{
	...
	while (fgets(buf, MAX_LINE, (FILE*)fp) != NULL)
	{
		...
		{
			vpr_printf(TIO_MESSAGE_ERROR,"str_to_long_int faild\n");
		}
		int find_i = 0;
...
// 建立block  与 p_log_brams队列中bram序号的对应关系
static void match_log_bram_id(BRAMS_log *p_log_brams)
{
	int iblk;
	int counter = 0;
...
static void load_log_pin_info(pin_dict * p_dict, BRAMS_log * p_log_brams)
{
	...
	for (i_bram_num = 0;i_bram_num<(*p_log_brams).BRAM_num;i_bram_num++)
	{
		...
		for(i_pin_dict = 0 ;i_pin_dict < (*p_dict).pin_num;i_pin_dict++)
			{
				if(strcmp((*p_log_brams).BRAM_LIST[i_bram_num].name,(*p_dict).pins[i_pin_dict].pin_name)==0)
				{
					((*p_log_brams).BRAM_LIST[i_bram_num].log_ratio) = ((*p_dict).pins[i_pin_dict].pin_ratio);
				}							
			}
